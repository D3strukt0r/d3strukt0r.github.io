{
  description = "d3strukt0r's portfolio";
  ...
  outputs = { self, nixpkgs, nix-utils }:
    ...
    in
    {
      ...
      packages = forAllSystems (pkgs:
        let
          # Impure: SEAL_DATA_KEY and DOCKER_LABELS_JSON are read from the
          # build environment. Different SEAL_DATA_KEY → different bundle →
          ...
          d3strukt0rs-portfolio = pkgs.stdenv.mkDerivation (finalAttrs: {
            ...
            pnpmDeps = pkgs.fetchPnpmDeps {
              ...
              pnpm = pkgs.pnpm_10;
              fetcherVersion = 3;
              hash = "sha256-x7GYDv3p8NcK8cq0cKHyJ23yaLJ5h1KTXYTddHzZfY0=";
            };
            ...
            installPhase = ''
              ...
              # Lay bundle at $out/opt/d3strukt0rs-portfolio/* so adding this
              # derivation to streamLayeredImage's `contents` puts it straight
              # at /opt/d3strukt0rs-portfolio/* in the image.
              mkdir -p $out/opt/d3strukt0rs-portfolio
              cp -r build node_modules package.json $out/opt/d3strukt0rs-portfolio/
          ...
          dockerImageStream = pkgs.dockerTools.streamLayeredImage {
            ...
            contents = [
              ...
              (pkgs.dockerTools.fakeNss.override {
                extraPasswdLines = [
                  "nonroot:x:65532:65532:nonroot:/opt/d3strukt0rs-portfolio:/sbin/nologin"
                ];
                extraGroupLines = [
                  "nonroot:x:65532:"
                ];
            ...
            config = {
              User = "65532:65532";
              WorkingDir = "/opt/d3strukt0rs-portfolio";
              Env = [
                "NODE_ENV=production"
                "PORT=3000"
                # .bin first so `react-router-serve` resolves via the pnpm shim.
                "PATH=/opt/d3strukt0rs-portfolio/node_modules/.bin:/bin:/usr/bin"
          ...
          dockerImage = pkgs.runCommand "d3strukt0rs-portfolio-image.tar" { } ''
            ${dockerImageStream} | ${fixHistoryScript} > $out
          '';
        in
        {
          inherit d3strukt0rs-portfolio dockerImage dockerImageStream;
          default = dockerImage;
        });
    };
}
