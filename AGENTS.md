...
- **`release.yml`-managed lines** — the `version = "X.Y.Z"; # x-release-please-version` line in `flake.nix` is bot-managed. Hand-edits get overwritten on the next release PR; bump out-of-band via `.github/release-please-manifest.json` instead.
- **TypeScript 6 and 7 run side by side** — `@typescript/native` *is* TypeScript 7 (aliased so it owns the `tsc` binary, which is what `pnpm run typecheck` executes), while the `typescript` entry is aliased to `@typescript/typescript6` so everything importing the `typescript` *module* gets a 6.x API: typescript-eslint via `@iwf-web/eslint-coding-standard`, plus `@react-router/dev` (peer `^5.1.0 || ^6.0.0`). TypeScript 7.0 deliberately ships no programmatic API — that arrives in 7.1 — so a plain `typescript@7` makes `pnpm run lint` crash on module load with `Cannot read properties of undefined`. Don't collapse the two entries until typescript-eslint supports 7. The 6.x compiler stays reachable as `tsc6`, and `pnpm peers check` warning about an unmet `typescript` peer is expected (peers match by package name, the alias resolves to `@typescript/typescript6`). TypeScript 7 also removed `baseUrl`, which is why `tsconfig.cloudflare.json` no longer sets it — `paths` resolve relative to the config file, so the `~/*` mapping is unaffected.
